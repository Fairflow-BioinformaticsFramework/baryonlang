...
USAGE_STR=$(echo -e "${YELLOW}<workdir>${RESET}" "${YELLOW}<genome>${RESET}" "${YELLOW}<scratch>${RESET}" "${GREEN}<bamsave>${RESET}")
...
if ! echo "${bamsave}" | grep -qE "^(true|false)$"; then
    errors+=("Invalid value for bamsave: ${bamsave}. Allowed: ['true', 'false']")
fi
